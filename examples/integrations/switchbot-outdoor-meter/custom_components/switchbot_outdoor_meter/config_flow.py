"""Config flow for the SwitchBot Outdoor Meter integration."""
...
class OutdoorMeterConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle a config flow for the SwitchBot Outdoor Meter."""
    ...
    async def async_step_user(
        ...
        for info in async_discovered_service_info(self.hass, connectable=False):
            if (
                ...
                or parse_service_info(info) is None
            ):
                continue
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(CONF_ADDRESS): SelectSelector(
                        SelectSelectorConfig(
                            options=[
                                SelectOptionDict(
                                    value=address, label=f"{info.name} ({address})"
                                )
